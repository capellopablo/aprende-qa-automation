...
export const addTimestampToEmail = (email) => {
  const split = email.split('@');
  return `${split[0]}_${getCurrentTimestamp()}@${split[1]}`;
}
...
export const getRandomCountry = () => {
  ...
  const countries = ['US', 'MX', 'CO', 'PE', 'AR', 'CL', 'UY'];
...
export const getRandomSchool = () => {
  ...
  const schools = ['Reposteria', 'Gastronomia', 'Oficios', 'Bienestar', 'Emprendimiento', 'Hospitalidad'];
